...
public class Deck {
    ...
    private int numberOfCards; //52 cards
     ...
     */
    public void shuffle() {
        for (int oldIndex = 0; oldIndex < numberOfCards; oldIndex++) 
        {
            int newIndex = random.nextInt(numberOfCards);
     ...
     */
    public Card deal() {
        if (nextCardIndex + 1 > numberOfCards) //used to say (nextCardIndex + 1 >= NO_OF_CARDS)
        {
    ...
    public ArrayList<Card> deal(int noOfCards) {
        ...
        if (nextCardIndex + noOfCards > numberOfCards) // used to say (nextCardIndex + noOfCards >= NO_OF_CARDS) 
        {
            throw new IllegalStateException("No cards left in deck");
        }
